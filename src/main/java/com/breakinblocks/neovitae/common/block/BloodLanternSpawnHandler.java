...
public final class BloodLanternSpawnHandler {
    ...
    private static void indexChunk(ServerLevel level, LevelChunk chunk) {
        ...
        for (int sectionIndex = 0; sectionIndex < sections.length; sectionIndex++) {
            ...
            for (int y = 0; y < 16; y++) for (int z = 0; z < 16; z++) for (int x = 0; x < 16; x++) {
                ...
                if (isLantern(state)) positions.add(BlockPos.asLong(chunk.getPos().getMinBlockX() + x, minY + y,
    ...
    private static final class LanternIndex {
        ...
        void add(BlockPos pos) { long key = ChunkPos.pack(pos.getX() >> 4, pos.getZ() >> 4); indexedChunks.add(key); positionsByChunk.computeIfAbsent(key, ignored -> new LongOpenHashSet()).add(pos.asLong()); }
